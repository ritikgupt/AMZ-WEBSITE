...
;(function($, window, document, undefined) {
  ...
  Owl.Workers = [ {
    ...
    run: function(cache) {
      ...
      var grid = !this.settings.autoWidth;
    ...
    run: function() {
      ...
      var view = Math.max(settings.items * 2, 4);
    ...
    run: function() {
      var rtl = this.settings.rtl ? 1 : -1;
      var padding = this.settings.stagePadding * 2;
      var begin = this.coordinates(this.current()) + padding;
      var end = begin + this.width() * rtl;
      var inner; var outer; var matches = []; var i; var n;
  ...
  Owl.prototype.update = function() {
    ...
    var filter = $.proxy(function(p) { return this[p]; }, this._invalidated);
    var cache = {};
...
;(function($, window, document, undefined) {
    ...
    this._handlers = {
      'initialized.owl.carousel change.owl.carousel resized.owl.carousel': $.proxy(function(e) {
        ...
        if ((e.property && e.property.name == 'position') || e.type == 'initialized') {
          ...
          var position = (e.property && e.property.value !== undefined ? e.property.value : this._core.current()) + i;
          var clones = this._core.clones().length;
          var load = $.proxy(function(i, v) { this.load(v); }, this);
  ...
  Lazy.prototype.load = function(position) {
    ...
    var $elements = $item && $item.find('.owl-lazy');
    ...
    $elements.each($.proxy(function(index, element) {
      var $element = $(element); var image;
      var url = (window.devicePixelRatio > 1 && $element.attr('data-src-retina')) || $element.attr('data-src') || $element.attr('data-srcset');
...
;(function($, window, document, undefined) {
  ...
  AutoHeight.prototype.update = function() {
    ...
    var lazyLoadEnabled = this._core.settings.lazyLoad;
    var visible = this._core.$stage.children().toArray().slice(start, end);
    var heights = [];
    var maxheight = 0;
...
;(function($, window, document, undefined) {
  ...
  Video.prototype.fetch = function(target, item) {
    ...
    var id = target.attr('data-vimeo-id') || target.attr('data-youtube-id') || target.attr('data-vzaar-id');
    var width = target.attr('data-width') || this._core.settings.videoWidth;
    var height = target.attr('data-height') || this._core.settings.videoHeight;
  ...
  Video.prototype.thumbnail = function(target, video) {
    ...
    var icon;
    var path;
    var dimensions = video.width && video.height ? 'width:' + video.width + 'px;height:' + video.height + 'px;' : '';
    var customTn = target.find('img');
    var srcType = 'src';
    var lazyClass = '';
    var settings = this._core.settings;
  ...
  Video.prototype.play = function(event) {
    ...
    var item = target.closest('.' + this._core.settings.itemClass);
    var video = this._videos[item.attr('data-video')];
    var width = video.width || '100%';
    var height = video.height || this._core.$stage.height();
    var html;
    var iframe;
...
;(function($, window, document, undefined) {
  ...
  Navigation.prototype.update = function() {
    var i; var j; var k;
    var lower = this._core.clones().length / 2;
    var upper = lower + this._core.items().length;
  ...
  Navigation.prototype.draw = function() {
    var difference;
    var settings = this._core.settings;
    var disabled = this._core.items().length <= settings.items;
    ...
    this._handlers = {
      ...
      'changed.owl.carousel': $.proxy(function(e) {
        if (e.namespace && e.property.name === 'position') {
          ...
          var hash = $.map(this._hashes, function(item, hash) {
